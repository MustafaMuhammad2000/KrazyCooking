...
describe('User Routes', function () {
  ...
  const res3 = await request(baseURL).patch('/updatePassword').send(body3).set('authorization', `${token}`);
  ...
  test('Save a post that is not saved already', async () => {
    ...
    const res = await request(baseURL).post('/savePost/' + postID).set('authorization', `${token}`);
    expect(res.header['content-type']).toBe('application/json; charset=utf-8');
    expect(res.statusCode).toBe(200);
  });
  ...
  test('View saved posts with at least 1 saved post', async () => {
    ...
    const res2 = await request(baseURL).get('/savedPosts').set('authorization', `${token}`);
    expect(res2.header['content-type']).toBe('application/json; charset=utf-8');
    expect(res2.statusCode).toBe(200);
  });
  ...
  test('Remove a post that the user does not have saved', async () => {
    ...
    const res = await request(baseURL).delete('/removedSavedPost/' + postID).set('authorization', `${token}`);
    expect(res.header['content-type']).toBe('application/json; charset=utf-8');
    expect(res.statusCode).toBe(404);
  });
  ...
  test('Remove a post that the user has saved', async () => {
    ...
    const res2 = await request(baseURL).delete('/removedSavedPost/' + postID).set('authorization', `${token}`);
    expect(res2.header['content-type']).toBe('application/json; charset=utf-8');
    expect(res2.statusCode).toBe(200);
  });
  ...
  test('Viewing my posts with at least one post created', async () => {
    ...
    const res = await request(process.env.BASEURL).post('/post').send(body).set('authorization', `${token}`);
    expect(res.header['content-type']).toBe('application/json; charset=utf-8');
    expect(res.statusCode).toBe(201);
    var postID = res.body.newPost

    // Then retrieving my posts
    const res2 = await request(baseURL).get('/myPosts').set('authorization', `${token}`);
    expect(res2.header['content-type']).toBe('application/json; charset=utf-8');
    expect(res2.statusCode).toBe(200);
  });
  ...
  .catch((err) => console.error("could not connect to mongo DB", err));
    ...
    const deleteUserPromise = await db.User.findOneAndDelete({ username: testUsername });
    Promise.all([deleteUserPromise]).then(() => {
      console.log('Deleted test user from database');
    });
    const deletePostPromise = await db.Post.findOneAndDelete({ title: 'Oreo Cereal' });
